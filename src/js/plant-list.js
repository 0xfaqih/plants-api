async function fetchPlantData() {
  try {
    const response = await fetch('http://47.245.87.80:9090/plant');
...
function displayPlantList(apiResponse) {
  ...
  if (apiResponse && apiResponse.status === 'success' && apiResponse.data && apiResponse.data.plants && Array.isArray(apiResponse.data.plants)) {
